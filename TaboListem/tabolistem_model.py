...
class TaboListem(object):
    ...
    def __init__(self):
        ...
        bert_pretrain_path='dmis-lab/biobert-base-cased-v1.2'#'allenai/scibert_scivocab_uncased'#'bert-base-cased'
        self.bert_pretrain_path=bert_pretrain_path

    
        self.tokenizer = transformers.BertTokenizerFast.from_pretrained(
            bert_pretrain_path, do_lower_case=False)#transformers.BertTokenizerFast
    ...
    def train(self, textfile, annotfile, runname):
        ...
        #tx_id = tf.convert_to_tensor(wordn_batch, dtype=object)
        tx_id = tf.convert_to_tensor(np.array([seq["wordn"] for seq in test], dtype = 'int32'))
        attention_mask = tf.convert_to_tensor(np.ones(tx_id.shape,dtype='int32'))
        tx2 = np.array([seq['ni'] for seq in test])
        Y_hat = np.array([[tobits(i, len(lablist)) for i in seq["bion"]] for seq in test])
        
        X = [tx_id, attention_mask, tx2]
        Y = model.predict(X)
        print('YES')
        #print(Y.shape())
        #print(mx.confusion_matrix(Y, Y_hat))   
        print(precision_score(Y, Y_hat, average = 'micro'))

        self.build_model()
        model=self.model
        model.load_weights('./TrainedModels/epoch_9_TaboListemModel_weights')

        wordn_batch=[seq["wordn"] for seq in test]
            
        tx2 = np.array([seq["ni"] for seq in test])
        ty = np.array( [np.asarray([tobits(i, len(lablist)) for i in seq["bion"]]).astype(np.int32) for seq in test])
        # ty = [0 0 0 0 1]
        wordn = tf.keras.preprocessing.sequence.pad_sequences(wordn_batch, padding = 'post')
        tx_id = tf.convert_to_tensor(wordn)
        attention_mask=tf.convert_to_tensor(np.ones(tx_id.shape, dtype='int32'))   
        #tx2 = tf.keras.preprocessing.sequence.pad_sequences(tx2, padding = 'post')
        tx_id = np.asarray(tx_id).astype(np.int32)
        attention_mask = np.asarray(attention_mask).astype(np.int32)
        #tx2 = np.array(tx2)
        tx2 = tf.keras.preprocessing.sequence.pad_sequences(tx2, padding = 'post')
        tx2 = np.asarray(tx2).astype(np.int32)
        ...
        CM = mx.confusion_matrix(ty[0], Y[0])     
        print(CM[0, 1])

        ty = tf.keras.preprocessing.sequence.pad_sequences(ty, padding = 'post')
        ty = np.asarray(ty).astype(np.int32) 
        ty = tf.convert_to_tensor(ty)       
        loss, accu = model.evaluate([tx_id, attention_mask, tx2], ty, verbose=0)
        #Y = model.predict()
        print('succussful evaluation! loss={}; accu={}'.format(loss, accu))
        ...
        jf = open("tabolistem_%s.json" % runname, "w", encoding="utf-8")#./TrainedModels/
        jf = open("tabolistem_%s.json" % runname, "w", encoding="utf-8")#./TrainedModels/
        json.dump(outjo, jf)
        jf.close()
        ...
        for epoch in range(10):
            ...
            #print("{} epoch got the loss: {} and accuracy :{}".format(epoch, loss, accu))
            print("Trained at", datetime.now(), "Loss", totloss / div, "Accuracy", totacc / div, file=sys.stderr)
            
            model.save_weights("./TrainedModels/epoch_%s_%s_weights" % (epoch, runname))
    ...
    def test(self, textfile, annotfile):
        ...
        for size in sizes:
            ...
            while batch_counter<len(train_l_d[size]):
                ...
                totf1 += f1 * size * batch_size
                div += size * batch_size
                print('\n precision: {}; f1-score: {}; recall: {}'.format(p,f1,r))
                '''
                yp_true.extend(np.array(Y[0], dtype = int))
                yp_test.extend(ty[0])
        ...
        print(cm)
        f1_macro = f1_score(y_true, y_test, average = 'macro')
        #f1_micro = f1_score(y_true, y_test, average = 'micro')
        f1_weighted = f1_score(y_true, y_test, average = 'weighted')
        pw = precision_score(y_true, y_test, average = 'weighted')
        pm = precision_score(y_true, y_test, average = 'macro')
        
        print('macro-F1-score={}'.format(f1_macro))
        print('weighted-F1-score={}'.format(f1_weighted))
        print('weighted-F1-score={}'.format(f1_weighted))
        print('weighted-precision={}'.format(pw))
        print('macro-precision={}'.format(pm))
        ...
        fpr['micro'], tpr['micro'], threshold = roc_curve(yp_true.ravel(), yp_test.ravel())
        roc_auc["micro"] = auc(fpr["micro"], tpr["micro"])
        
        print("The approperiate threshold is {}".format(roc_auc_score(yp_true.ravel(), yp_test.ravel())))
        lw=2
        plt.figure()
